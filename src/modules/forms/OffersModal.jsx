...
import OfferDecision from "./OfferDecision"; // Asegúrate de importar el componente
...
const OffersModal = ({ requestId, onClose }) => {
  ...
  const itemsPerPage = 6; // Cantidad de ofertas por página
  ...
  // Filtrar las ofertas para que no se muestren las aceptadas ni rechazadas
  const filteredOffers = offers.filter(
    (offer) => offer.status !== "aceptada" && offer.status !== "rechazada"
  );

  // Ofertas visibles en la página actual (después del filtro)
  ...
  const refreshOffers = async () => {
    // Refrescar las ofertas después de una decisión
    try {
      const token = document.cookie;
      const response = await axios.get(
        `https://hackaton-back-production.up.railway.app/request/${requestId}`,
  ...
  return (
    <div className="fixed inset-0 bg-opacity-75 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg p-6 w-[90%] max-w-7xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-3 gap-4">
              {currentOffers.map((offer) => (
                ...
                >
                  ...
                  {/* Agregar el componente OfferDecision para aceptar/rechazar la oferta */}
                  <OfferDecision
                    offerId={offer._id}
                    requestId={requestId}
                    onDecisionMade={refreshOffers} // Llamamos a la función para actualizar las ofertas
                  />
                </div>
              ))}
            </div>
